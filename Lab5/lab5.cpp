...
void ReadInput(char *inputName, vector<ComplexNum> &list){  // input 파일 읽고 list에 저장
    ...
    string sline;
    getline(fin, sline, ',');   // 첫번째 줄의 '실수부' 처리
    getline(fin, sline);        // 첫번째 줄의 '허수부' 처리
...
void WriteOutput1(char *outputName1, vector<ComplexNum> &list){ // input 파일을 그대로 복사해서 출력하는 함수
    ofstream fout(outputName1);
    if ( !fout.is_open() ) {    // 파일 오픈 실패 시 오류 메시지 출력하고 함수 종료
        cout << outputName1 << " 이 열리지 않음" << endl;
        return;
    }
    
    fout << "실수부" << ',' << "허수부" << endl;  // 첫번째 줄의 실수부, 허수부 문자열 입력
    for ( int i = 0; i < list.size(); i++ )   // 데이터 출력
        fout << list[i].getReal() << ',' << list[i].getImagin() << endl;
    fout.close();
}
void WriteOutput2(char *outputName2, vector<ComplexNum> &list){ // input 파일에서 읽은 list를 정렬해서 출력하는 함수
    ofstream fout(outputName2);
    if ( !fout.is_open() ) {    // 파일 오픈 실패 시 오류 메시지 출력하고 함수 종료
        cout << outputName2 << " 이 열리지 않음" << endl;
        return;
    }
